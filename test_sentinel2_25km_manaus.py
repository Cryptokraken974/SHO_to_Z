#!/usr/bin/env python3
"""
🛰️ SENTINEL-2 25KM OPTIMAL CONFIGURATION TEST - MANAUS LOCATION
===============================================================

Test Sentinel-2 data with 25km configuration using Manaus, Brazil 
(known to have good Sentinel-2 coverage) to validate the optimal methodology.

Location: Manaus, Brazil (-3.1°, -60.0°) - Amazon Basin city with good coverage
Buffer: 12.5km radius (25x25km area) - Same as elevation test
Objective: Get highest resolution Sentinel-2 GeoTIFF using proven methodology
"""
...
async def test_sentinel2_25km_manaus():
    """Test Sentinel-2 with optimal 25km configuration in Manaus"""
    
    print("🛰️ SENTINEL-2 25KM OPTIMAL CONFIGURATION TEST")
    print("=" * 60)
    print("📍 Using Manaus, Brazil (proven Sentinel-2 coverage)")
    print("🔓 Using Microsoft Planetary Computer (no authentication required)")
    print("📏 Same 25km methodology as successful elevation test")
    print()
    
    # Manaus coordinates (confirmed to have Sentinel-2 data)
    lat, lng = -3.1, -60.0  # Manaus, Brazil - Amazon Basin
    buffer_km = 12.5  # 25km x 25km area (same as elevation test)
    
    print(f"📍 Target location: Manaus, Brazil (Amazon Basin)")
    print(f"📐 Coordinates: {lat}°, {lng}°")
    print(f"📏 Buffer radius: {buffer_km}km (Total area: 25x25km)")
    print(f"🎯 Methodology: Same as successful elevation test")
    print()
    ...
    try:
        ...
        if result.success:
            ...
            print(f"📊 Reported size: {result.file_size_mb:.2f} MB")
            print()
        ...
        print("-" * 40)
        print("Elevation test (25km area, Amazon Basin):")
        print("  • File size: ~13,500 KB (13.5 MB)")
        print("  • Resolution: 1800x1800+ pixels")
        print("  • Success rate: 100%")
        print("  • Methodology: 25km buffer (12.5km radius)")
        print()
        ...
        if result.success and result.file_path:
            ...
            if file_path.exists():
                ...
                print(f"Sentinel-2 test (25km area, Manaus):")
                ...
                print(f"  • Methodology: Same 25km buffer")
        ...
        if result.success:
            ...
            print("🏆 25km methodology produces high-quality satellite imagery")
            print("🌎 Validated optimal configuration for Amazon Basin imagery")
            
            if analysis.get('size_mb', 0) > 10:
                print("🎖️ ACHIEVEMENT: High-resolution GeoTIFF successfully acquired")
        else:
            print("❌ Sentinel-2 25km configuration test FAILED")
            print("🔄 May need to retry or adjust parameters")
...
if __name__ == "__main__":
    print("🚀 Starting Sentinel-2 25km optimal configuration test (Manaus)...")
    print()
    
    try:
        asyncio.run(test_sentinel2_25km_manaus())
