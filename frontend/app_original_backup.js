$(function () {
  ...
  function initializeMap() {
    ...
    // Note: LAZ file markers will be created dynamically when files are loaded
    console.log('*** Map initialization complete - LAZ markers will be added when files load ***');
  ...
  // Get LIDAR Data Button - Fetch LIDAR data from provider
  $('#get-lidar-btn').on('click', function() {
    let lat = $('#lat-input').val();
    let lng = $('#lng-input').val();
    
    // If no coordinates are set, use Portland, Oregon as default (good LIDAR coverage)
    if (!lat || !lng || lat === '' || lng === '') {
      lat = 45.5152;  // Portland, Oregon
      lng = -122.6784;
      ...
      if (map) {
        ...
        // Add a marker
        if (window.lidarTestMarker) {
          map.removeLayer(window.lidarTestMarker);
        }
        window.lidarTestMarker = L.marker([lat, lng]).addTo(map)
          .bindPopup('LIDAR Data Request Location<br>Lat: ' + lat + '<br>Lng: ' + lng)
          .openPopup();
      }
      
      alert('Using Portland, Oregon coordinates for LIDAR data request.');
    }
    ...
    acquireLidarData(latNum, lngNum);
  });
  ...
  $('#select-file').on('click', function() {
    const selectedFileItem = $('.file-item.selected');
    const selectedFile = selectedFileItem.data('file');
    ...
    if (selectedFile) {
      ...
      // Try to center map on the selected file using coordinate metadata
      const lat = selectedFileItem.data('lat');
      const lng = selectedFileItem.data('lng');
      
      if (lat && lng && map) {
        console.log(`*** Centering map on file: ${selectedFile} at coordinates: ${lat}, ${lng} ***`);
        map.setView([lat, lng], 13);
        
        // Remove existing location pin if any
        if (currentLocationPin) {
          map.removeLayer(currentLocationPin);
        }
        
        // Add a new location pin for the selected LAZ file
        currentLocationPin = L.marker([lat, lng])
          .addTo(map)
          .bindPopup(`Selected LAZ File<br>${selectedFile.split('/').pop()}<br>Lat: ${lat}<br>Lng: ${lng}`)
          .openPopup();
          
        console.log(`*** Location pin added for LAZ file at ${lat}, ${lng} ***`);
      } else {
        console.log(`*** No coordinate metadata available for file: ${selectedFile} ***`);
      }
    }
  });
  ...
  function loadFiles() {
    $.get('/api/list-laz-files', function(data) {
      ...
      data.files.forEach(function(fileInfo) {
        const filePath = fileInfo.path || fileInfo; // Handle both old and new format
        const displayName = filePath.split('/').pop();
        const fileItem = $(`<div class="file-item" data-file="${filePath}">${displayName}</div>`);
        
        // Store coordinate metadata if available
        if (fileInfo.center_lat && fileInfo.center_lng) {
          fileItem.data('lat', fileInfo.center_lat);
          fileItem.data('lng', fileInfo.center_lng);
        }
        
        fileList.append(fileItem);
      });
      
      // Create map markers for LAZ files if map is available
      if (map && data.files.length > 0) {
        createLazFileMarkers(data.files);
      }
    }).fail(function() {
  ...
  function showProgressDisplay(title = 'Data Download Progress') {
    if (progressDisplay) {
  ...
  function updateProgressDisplay(data) {
    ...
    switch (data.type) {
      ...
      case 'download_progress':
        ...
        updateOrAddProgressItem(data.band, progressText, '⬇️', progressPercent);
        break;
      ...
      case 'download_complete':
        ...
        const completionText = data.final_size_mb ? 
          ...
          'Download complete!';
        updateOrAddProgressItem(data.band, completionText, '✅', 100);
        break;
      ...
      // LIDAR-specific progress cases
      case 'lidar_acquisition_started':
        status.text('Starting LIDAR acquisition...');
        addProgressItem('LIDAR', `Starting acquisition for ${data.region || 'region'}`, '📡');
        break;
        
      case 'provider_selected':
        status.text(`Provider selected: ${data.provider}`);
        updateOrAddProgressItem('LIDAR', `Using provider: ${data.provider}`, '🏢');
        break;
        
      case 'cache_hit':
        status.text('Found cached LIDAR data');
        updateOrAddProgressItem('LIDAR', 'Using cached data', '💾', 100);
        break;
        
      case 'download_started':
        status.text(`Starting ${data.provider} download...`);
        updateOrAddProgressItem('LIDAR', `Starting ${data.provider} download...`, '⬇️');
        break;
        
      case 'download_progress':
        if (data.progress) {
          status.text(`Downloading LIDAR: ${data.progress}%`);
          updateOrAddProgressItem('LIDAR', `${data.progress}% downloaded`, '⬇️', data.progress);
        } else {
          status.text('Downloading LIDAR data...');
          updateOrAddProgressItem('LIDAR', data.message || 'Downloading...', '⬇️');
        }
        break;
        
      case 'download_completed':
        status.text('LIDAR download completed');
        updateOrAddProgressItem('LIDAR', 'Download completed', '✅', 100);
        break;
        
      case 'processing_started':
        status.text('Processing LIDAR files...');
        updateOrAddProgressItem('LIDAR', 'Processing files...', '⚙️');
        break;
        
      case 'processing_completed':
        status.text('LIDAR processing completed');
        updateOrAddProgressItem('LIDAR', data.message || 'Processing completed', '✅', 100);
        break;
        
      case 'lidar_acquisition_completed':
        status.text(`LIDAR acquisition completed for ${data.region}`);
        updateOrAddProgressItem('LIDAR', `Acquisition completed: ${data.files?.length || 0} files`, '🎉', 100);
        break;
        
      case 'lidar_acquisition_error':
        status.text(`LIDAR error: ${data.message}`);
        updateOrAddProgressItem('LIDAR', `Error: ${data.message}`, '❌');
        break;
      ...
      content.append(`
        ...
        ">
          <div style="display: flex; align-items: center; margin-bottom: 8px;">
            <span class="progress-icon" style="margin-right: 8px; font-size: 16px;">${icon}</span>
            <strong style="margin-right: 8px;">${band}:</strong> 
  ...
  function updateOrAddProgressItem(band, message, icon, progressPercent = null) {
    const itemId = `progress-${band}`;
    ...
    if (existingItem.length > 0) {
      ...
      if (progressPercent !== null) {
        const progressContainer = existingItem.find('.progress-bar-container');
        const progressBar = existingItem.find('.progress-bar');
        
        // Show progress bar if it's hidden
        progressContainer.show();
        
        // Update progress bar width
        progressBar.css('width', `${Math.min(100, Math.max(0, progressPercent))}%`);
        
        // Change color based on progress
        if (progressPercent >= 100) {
          progressBar.css('background', 'linear-gradient(90deg, #28a745, #20c997)');
        } else {
          progressBar.css('background', 'linear-gradient(90deg, #00bfff, #0099cc)');
        }
      }
    } else {
      addProgressItem(band, message, icon);
      // If we have progress data, update the newly created item
      if (progressPercent !== null) {
        updateOrAddProgressItem(band, message, icon, progressPercent);
      }
    }
  ...
  async function downloadSentinel2Images(lat, lng) {
    ...
    showProgressDisplay('🛰️ Sentinel-2 Download Progress');
  ...
  // Function to acquire LIDAR data from provider
  async function acquireLidarData(lat, lng) {
    console.log(`*** Acquiring LIDAR data for ${lat}, ${lng} ***`);
    
    // Show progress display first
    showProgressDisplay('📡 LIDAR Data Acquisition Progress');
    
    try {
      // Initialize WebSocket connection and wait for it to be ready
      console.log('*** Establishing WebSocket connection... ***');
      await initializeProgressWebSocket();
      console.log('*** WebSocket ready, starting LIDAR acquisition... ***');
      
      // Update initial status
      $('#progress-status').text('WebSocket connected, preparing LIDAR data request...');
    ...
    // Show loading message on button
    const button = $('#get-lidar-btn');
    const originalText = button.text();
    button.text('📡 Acquiring...').prop('disabled', true);
    
    // Create AbortController for cancellation
    currentDownloadController = new AbortController();
    
    try {
      $('#progress-status').text('Sending LIDAR data request...');
      
      const response = await fetch('/api/acquire-lidar', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          provider: 'auto'  // Auto-select best provider based on location
        }),
        signal: currentDownloadController.signal  // Add abort signal
      });
      ...
      if (response.ok) {
        ...
        if (result.success) {
          // Show success message for acquisition
          showSuccessMessage(`Successfully acquired LIDAR data! 
            Provider: ${result.metadata.provider || 'Unknown'}
            File size: ${result.file_size_mb ? result.file_size_mb.toFixed(2) + ' MB' : 'Unknown'}
            Resolution: ${result.metadata.resolution || 'Unknown'}
            Point density: ${result.metadata.point_density || 'Unknown'}`);
          
          console.log('*** LIDAR acquisition successful:', result);
          ...
          // Automatically process and display the LIDAR data
          const regionName = result.region_name;
          if (regionName) {
            console.log(`*** Processing LIDAR data for region: ${regionName} ***`);
            await processAndDisplayLidar(regionName);
          } else {
            console.error('*** No region name found in LIDAR result ***');
            console.log('*** Full LIDAR result structure:', result);
          }
        } else {
          showErrorMessage('LIDAR acquisition failed: ' + result.error_message);
          hideProgressDisplay();
        }
      } else {
        const errorText = await response.text();
        showErrorMessage('Failed to acquire LIDAR data: ' + errorText);
        hideProgressDisplay();
      }
    } catch (error) {
      console.error('*** Error acquiring LIDAR data:', error);
      
      // Handle different error types
      if (error.name === 'AbortError') {
        console.log('*** LIDAR acquisition was aborted by user ***');
        // Don't show error message for user-initiated cancellation
        if (!downloadAborted) {
          showErrorMessage('LIDAR acquisition was cancelled');
        }
      } else {
        showErrorMessage('Error acquiring LIDAR data: ' + error.message);
      }
  ...
  // Function to process and display LIDAR data
  async function processAndDisplayLidar(regionName) {
    console.log(`*** Processing LIDAR data for region: ${regionName} ***`);
    
    try {
      // Process LIDAR files (convert to LAZ format if needed)
      const formData = new FormData();
      formData.append('region_name', regionName);
      
      const response = await fetch('/api/process-lidar', {
        method: 'POST',
        body: formData
      });
      ...
      if (response.ok) {
        ...
        if (result.success) {
          console.log('*** LIDAR processing successful:', result);
          showSuccessMessage(`LIDAR data processed successfully! 
            Files: ${result.files.length} LAZ files ready for terrain analysis`);
          
          // Refresh the LAZ file list to show the new files
          loadFiles();
          
        } else {
          const errorMsg = result.error || result.error_message || 'Unknown error';
          console.error('*** LIDAR processing failed:', errorMsg);
          showErrorMessage('Failed to process LIDAR data: ' + errorMsg);
        }
      } else {
        const errorText = await response.text();
        console.error('*** LIDAR processing request failed:', errorText);
        showErrorMessage('Failed to process LIDAR data: ' + errorText);
      }
    } catch (error) {
      console.error('*** Error processing LIDAR data:', error);
      showErrorMessage('Error processing LIDAR data: ' + error.message);
    }
  }
  ...
  // Function to create map markers for LAZ files
  function createLazFileMarkers(files) {
    console.log('*** Creating dynamic LAZ file markers ***');
    
    // Clear existing markers (except the current location pin)
    if (window.lazFileMarkers) {
      window.lazFileMarkers.forEach(marker => {
        map.removeLayer(marker);
      });
    }
    window.lazFileMarkers = [];
    ...
    files.forEach(function(fileInfo) {
      ...
      if (fileInfo.center_lat && fileInfo.center_lng) {
        console.log(`*** Adding marker for ${fileName} at ${fileInfo.center_lat}, ${fileInfo.center_lng} ***`);
        
        const marker = L.marker([fileInfo.center_lat, fileInfo.center_lng])
          .addTo(map)
          .bindPopup(`LAZ File: ${fileName}<br>Lat: ${fileInfo.center_lat}<br>Lng: ${fileInfo.center_lng}`)
          .on('click', function() {
            selectedLazFile = filePath;
            $('#selected-file-name').text(fileName)
                                     .removeClass('text-[#666]').addClass('text-[#00bfff]');
            
            // Center map on the clicked marker coordinates
            map.setView([fileInfo.center_lat, fileInfo.center_lng], 13);
            
            // Update current location pin
            if (currentLocationPin) {
              map.removeLayer(currentLocationPin);
            }
            currentLocationPin = L.marker([fileInfo.center_lat, fileInfo.center_lng])
              .addTo(map)
              .bindPopup(`Selected LAZ File<br>${fileName}<br>Lat: ${fileInfo.center_lat}<br>Lng: ${fileInfo.center_lng}`)
              .openPopup();
          });
          
        window.lazFileMarkers.push(marker);
      }
    });
    
    console.log(`*** Created ${window.lazFileMarkers.length} LAZ file markers ***`);
  }
