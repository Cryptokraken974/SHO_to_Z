...
class OpenTopographySource(BaseDataSource):
    ...
    async def download(self, request: DownloadRequest) -> DownloadResult:
        ...
        try:
            ...
            if request.data_type == DataType.LAZ:
                return await self._download_point_cloud(request, datasets, cache_path)
            else:
                return await self._download_elevation(request, datasets, cache_path)
    ...
    async def _download_point_cloud(self, request: DownloadRequest, datasets: List[Dict], cache_path: Path) -> DownloadResult:
        """Download point cloud data using PDAL."""
        try:
            ...
            await self._execute_pipeline_async(pipeline)
    ...
    async def _download_elevation(self, request: DownloadRequest, datasets: List[Dict], cache_path: Path) -> DownloadResult:
        """Download elevation data (DTM/DEM) using PDAL."""
        try:
            # Create AOI polygon in Web Mercator (EPSG:3857)
            aoi_3857 = await self._create_aoi_polygon(request.bbox)
