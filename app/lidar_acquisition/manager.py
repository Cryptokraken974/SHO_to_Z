...
class LidarAcquisitionManager:
    ...
    async def acquire_lidar_data(
        ...
        try:
            ...
            # Create region output directory if it doesn't exist
            region_dir = self.output_dir / region_name
            region_dir.mkdir(parents=True, exist_ok=True)
            
            # Calculate bounding box for requested bounds
            from ..data_acquisition.utils.coordinates import CoordinateConverter
            coordinate_converter = CoordinateConverter()
            bbox = coordinate_converter.create_bounding_box(lat, lng, buffer_km)
            ...
            with open(metadata_path, 'w') as f:
                ...
                f.write(f"# Generated by LidarAcquisitionManager.acquire_lidar_data()\n")
            
            if progress_callback:
                await progress_callback({
                    "type": "metadata_created",
                    "message": f"Created immediate metadata.txt with requested bounds",
