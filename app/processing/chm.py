...
def chm(input_file: str, region_name: str = None) -> str:
    ...
    # Generate output filename: <file_stem>_CHM.tif (add _clean suffix if quality mode)
    output_filename = f"{file_stem}_CHM"
    ...
    if quality_mode_used:
        print(f"✨ Quality mode: Clean CHM will be generated from clean DTM and DSM")
    ...
    try:
        ...
        dsm_path = dsm(actual_input_file, region_name)
        ...
        # 🎯 QUALITY MODE PNG GENERATION: Generate viridis colorized PNG for clean CHM
        if quality_mode_used:
            print(f"\n🖼️ QUALITY MODE: Generating viridis colorized PNG for clean CHM")
            try:
                ...
                print(f"✅ Quality mode CHM PNG file created: {png_path}")
                logger.info(f"Quality mode CHM PNG generated: {png_path}")
            except Exception as png_error:
                print(f"⚠️ Quality mode CHM PNG generation failed: {png_error}")
                logger.warning(f"Quality mode CHM PNG generation failed: {png_error}")
